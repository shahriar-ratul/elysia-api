...
logger.info({ port }, 'Starting server on port ${port}')

const app = new Elysia()
  .use(
    env({
      DATABASE_URL: t.String(),
      PORT: t.Number(), // Use t.Numeric() for numeric env vars
    })
  )
  ...
  .onAfterResponse(({ requestId, logger, request, set }) => {
    logger.info({ requestId, method: request.method, url: request.url, statusCode: set.status }, 'Request completed')
  })
  ...
  .group('/api/v2', (app) =>
    app
      ...
      .use(roleRoutes)
      .use(permissionRoutes)
    // Add v2 specific routes or overrides here
  )
